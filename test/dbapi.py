...
class ModuleTests(unittest.TestCase):
    ...
    def CheckThreadSafety(self):
        self.assertEqual(
            sqlite.threadsafety, 1, "threadsafety is %d, should be 1" % sqlite.threadsafety
        )
    ...
    def CheckWarning(self):
        self.assertTrue(
            issubclass(sqlite.Warning, Exception), "Warning is not a subclass of Exception"
        )

    def CheckError(self):
        self.assertTrue(issubclass(sqlite.Error, Exception), "Error is not a subclass of Exception")
...
class ConnectionTests(unittest.TestCase):
    ...
    @unittest.skipIf(
        sqlite.sqlite_version_info >= (3, 3, 1), "needs sqlite versions older than 3.3.1"
    )
...
class BlobTests(unittest.TestCase):
    ...
    def CheckBlobWrite(self):
        ...
        self.assertEqual(
            self.cx.execute("select blob_col from test").fetchone()[0], self.second_data
        )
    ...
    def CheckBlobWriteWhenReadOnly(self):
        read_only_blob = self.cx.open_blob("test", "blob_col", 1, readonly=True)
        with self.assertRaises(sqlite.OperationalError):
    ...
    def CheckBlobSetItem(self):
        self.blob[0] = b"b"
        self.assertEqual(
            self.cx.execute("select blob_col from test").fetchone()[0], b"b" + self.blob_data[1:]
        )
...
class SqliteColumnTypeTests(unittest.TestCase):
    ...
    def CheckDeclTypes(self):
        ...
        self.assertEqual(curs.description, (("c", None, None, None, None, None, None),))
